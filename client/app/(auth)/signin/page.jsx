...
export default function SignInPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await apiClient.post("/auth/signin", userData);

      toast.success(res?.message || "Signin successful, welcome!");

      // Sync guest cart items stored in LocalStorage to the backend database
      ...
      const message = error?.data?.message || error?.message || "Signin failed";
      const updatedMessage = message.toLowerCase();
